...
![Diagram comparing two possible NVDLA systems. The 'Small' NVDLA system shows a CPU connected to an NVDLA block containing IRQ, CSB, SRAMIF (dashed line), and DBBIF. The NVDLA block connects to Sys DRAM. The 'Large' NVDLA system shows a CPU connected to a Microcontroller, which connects to an NVDLA block containing IRQ, CSB, SRAMIF, and DBBIF. The NVDLA block connects to a separate SRAM block and Sys DRAM.](3cab54230d8ae3b786ddda81346602cc_img.jpg)

Diagram comparing two possible NVDLA systems. The 'Small' NVDLA system shows a CPU connected to an NVDLA block containing IRQ, CSB, SRAMIF (dashed line), and DBBIF. The NVDLA block connects to Sys DRAM. The 'Large' NVDLA system shows a CPU connected to a Microcontroller, which connects to an NVDLA block containing IRQ, CSB, SRAMIF, and DBBIF. The NVDLA block connects to a separate SRAM block and Sys DRAM.
...
![Figure 2: Internal architecture of NVDLA core. The diagram shows a block diagram of the NVDLA core. The core is divided into a Configuration interface block and a Memory interface block. The Configuration interface block connects to the CSB / interrupt interface. The Memory interface block connects to the DDB interface and the Second DDB interface (optional). The Memory interface block is connected to the Convolution buffer, which is connected to the Convolution core. The Convolution core is connected to the Activation engine (SDP). The Activation engine (SDP) is connected to the Pooling engine (PDP). The Pooling engine (PDP) is connected to the Local resp. norm (CDP). The Local resp. norm (CDP) is connected to the Reshape (RUBIK). The Reshape (RUBIK) is connected to the Bridge DMA.](7a3561af571faf036baa93f5f4b1bdb9_img.jpg)

Figure 2: Internal architecture of NVDLA core. The diagram shows a block diagram of the NVDLA core. The core is divided into a Configuration interface block and a Memory interface block. The Configuration interface block connects to the CSB / interrupt interface. The Memory interface block connects to the DDB interface and the Second DDB interface (optional). The Memory interface block is connected to the Convolution buffer, which is connected to the Convolution core. The Convolution core is connected to the Activation engine (SDP). The Activation engine (SDP) is connected to the Pooling engine (PDP). The Pooling engine (PDP) is connected to the Local resp. norm (CDP). The Local resp. norm (CDP) is connected to the Reshape (RUBIK). The Reshape (RUBIK) is connected to the Bridge DMA.
...
- **Data Backbone (DDB) interface.** The DDB interface connects NVDLA and the main system memory subsystems. It is a synchronous, high-speed, and highly configurable data bus. It can be specified to have different address sizes, different data sizes, and to issue different sizes of requests depending upon the requirements of the system. The data backbone interface is a simple interface protocol that is similar to AXI (and can be readily used in AXI-compliant systems).

The DDB interface has an optional second connection which can be used when there is a second memory path available. This connection is identical in design to the primary DDB interface and is intended for use with an on-chip SRAM that can provide higher throughput and lower access latency. The second DDB interface is not necessary for NVDLA to function, systems that do not require this memory interface can save area by removing it.
...
The PDP maps onto the `tf.nn.avg_pool`, `tf.nn.max_pool`, and `tf.nn.pool` operations.
...
NVDLA has a full software ecosystem supporting it. Part of this ecosystem includes the on-device software stack, a part of the NVDLA open source release; additionally, NVIDIA will provide a full training infrastructure to build new models that incorporate Deep Learning, and to convert existing models to a form that is usable by NVDLA

software. In general, the software associated with NVDLA is grouped into two groups: the *compilation tools* (model conversion), and the *runtime environment* (run-time software to load and execute networks on NVDLA). The general flow of this is as shown in the figure below; and each of these is described below.

![Dataflow diagram showing the flow of software inside NVDLA. DL training software feeds into a 'Build' box containing Parser, Compiler, and Optimizer. The output of the Build box feeds into User-mode driver (UMD). UMD feeds into Kernel-mode driver (KMD). KMD feeds into NVDLA. The diagram is labeled (_images/nvdla-primer-sw-flow.svg).](d864789b0d8384da1d22fd6a5d76bbdf_img.jpg)

Dataflow diagram showing the flow of software inside NVDLA. DL training software feeds into a 'Build' box containing Parser, Compiler, and Optimizer. The output of the Build box feeds into User-mode driver (UMD). UMD feeds into Kernel-mode driver (KMD). KMD feeds into NVDLA. The diagram is labeled (\_images/nvdla-primer-sw-flow.svg).
...
- **User Mode Driver.** The main interface with user-mode programs. After parsing the neural network compiler compiles network layer by layer and converts it into a file format called NVDLA Loadable (glossary.html#term-NVDLA-Loadable). User mode runtime driver loads this loadable and submits inference job to Kernel Mode Driver (sw/runtime\_environment.html#kernel-mode-driver)
...
![Diagram illustrating the Portability layers in the NVDLA system. The diagram shows two main software stacks. The top stack, running in User application software, consists of a Portability layer (written by integrator) and a DLA core (from NVDLA GitHub; regularly updated). The bottom stack, running in the Operating system kernel, also consists of a Portability layer (written by integrator) and a DLA core (from NVDLA GitHub; regularly updated). The DLA core in the bottom stack is connected to DLA hardware. An arrow points from the DLA core in the top stack down to the DLA core in the bottom stack.](891ff9b651838b7f59e9a1612a739e15_img.jpg)

Diagram illustrating the Portability layers in the NVDLA system. The diagram shows two main software stacks. The top stack, running in User application software, consists of a Portability layer (written by integrator) and a DLA core (from NVDLA GitHub; regularly updated). The bottom stack, running in the Operating system kernel, also consists of a Portability layer (written by integrator) and a DLA core (from NVDLA GitHub; regularly updated). The DLA core in the bottom stack is connected to DLA hardware. An arrow points from the DLA core in the top stack down to the DLA core in the bottom stack.
...
Power and performance in the following table are shown for a 1GHz frequency. Power and performance for a given configuration can be varied though adjustment of voltage and frequency.

| # MACs | Conv. buffer size (KB) | SDRAM bandwidth (GB/s) | Silicon Cell Area ( $\text{mm}^2$ , 28nm) | Silicon Cell Area ( $\text{mm}^2$ , 16nm) | Int8 ResNet-50 (frames/sec) | Power Estimate Peak/Average (mW, 16nm) |
|--------|------------------------|------------------------|-------------------------------------------|-------------------------------------------|-----------------------------|----------------------------------------|
...
- NVIDIA Webinar Series: Deep Learning Demystified (<https://info.nvidia.com/deep-learning-demystified.html>)
- A Beginner's Guide to Understanding Convolutional Neural Networks
  - Part 1 (<https://adeshpande3.github.io/A-Beginner's-Guide-To-Understanding-Convolutional-Neural-Networks/>)
...
- NVIDIA Whitepaper: GPU-Based Deep Learning Inference: A performance and Power Analysis ([https://www.nvidia.com/content/tegra/embedded-systems/pdf/jetson\\_tx1\\_whitepaper.pdf](https://www.nvidia.com/content/tegra/embedded-systems/pdf/jetson_tx1_whitepaper.pdf))
...
- Inception (GoogLeNet): Going Deeper with Convolutions ([https://www.cv-foundation.org/openaccess/content\\_cvpr\\_2015/papers/Szegedy\\_Going\\_Deeper\\_With\\_2015\\_CVPR\\_paper.pdf](https://www.cv-foundation.org/openaccess/content_cvpr_2015/papers/Szegedy_Going_Deeper_With_2015_CVPR_paper.pdf))
