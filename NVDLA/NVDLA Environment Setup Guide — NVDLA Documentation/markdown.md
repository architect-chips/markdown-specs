...
Any project name specified should have a project spec file located at  
hw/spec/defs/<project\_name>.spec.
...
Enter to determine designware_noexist (Press ENTER 0 to use design ware, 1 to not use d
...
After this setup, a file named `tree.make` will be generated under TOT directory.
...
If no designware implementation is available, the NVDLA repository contains an alternate implementation. The design can be compiled to using the alternate implementation by setting the tree.make variable DESIGNWARE\_NOEXIST to 1.

```
DESIGNWARE_NOEXIST := 1
```

If designware implementation is available, set the tree.make variable DESIGNWARE\_NOEXIST to 0. When using a designware implementation, the designware directory also needs up be added to tree.make with the DESIGNWARE\_DIR variable. This variable should point to a directory where the simulation version of the designware component is available. Synthesis will pick up an internal optimized version.
...
There will be a new directory named outdir under TOT. RTL file can be found under TOT/outdir/<project\_name>/vmod.

### Build Test Bench

```
TOT > ./tools/bin/tmake -build_ready_for_test
```
...
To validate tree healthiness after download from git server, or to make sure changes are not break base line functions, use following command to build tree and then run protection tests

```
TOT > ./tools/bin/tmake -build_verif_protection
```
...
There is a tmake target for the Cmodel of NVDLA, which lives in the `cmod/` directory. If building the Virtual Platform, or another application that uses the NVDLA Cmodel, please use following command

```
TOT > ./tools/bin/tmake -build_cmod_top
```
...
CMOD headers and dynamically linked shared object library could be found under `outdir/<project_name>/cmod/release`
